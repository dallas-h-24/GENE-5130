...
**Side Quest** From Monday Excercise, open a text editor or even excel! And create a metadata file from the small dataset that you ran monday (solution posted on canvas, feel free to run it if you need to do that again!)
...
## Tell dds to use control
relevel(dds$dexamethasone, ref = 'control')
## you can do this if the baseline is wildtype or whatever else
```
...
#res willing to accept 5% false positive
```

Why do I need to correct for multiple comparisons? How can I do it? - bonferroni (divide p-value by \# of samples) - benhamini halberg (FDR)

```{r}
res
```

## the - log fold change means it is underrepresented in the treatment, vice versa for positive log

## fold change

## counts from deseq 2 takes library composition into account (compared the RPKM and FPKM)

### And that's it! you have a set of genes and how differentially expressed these genes are!

#first p-value: compares control and treated for each sample, bonferroni divide the p-value by the \##
